...
rowSize = len(input_arr)
colSize = len(input_arr[0])
...
for key, value in numbers.items():
  neighbours = get_neighbours(key, colSize, rowSize)
...
for i in range(rowSize):
  for j in range(colSize):
    if input_arr[i][j]== '*':
      neighbours = get_neighbours([i, (i,j)], colSize, rowSize)
      neighbour_numbers = []
